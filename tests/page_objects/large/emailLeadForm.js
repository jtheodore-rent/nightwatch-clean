const leadFormContainerSelector = '[data-test-id=lead-modal]'
...
const moveDateInputSelector = `${leadFormContainerSelector} input[name=moveDate]`
...
const leadFormCommands = {
  verifyHeading: leadFormContainer => {
    ...
    leadFormContainer.expect.element('@propertyName').to.be.visible.after(0)
  },
  ...
  verifyMoveDateInput: (leadFormContainer, moveDate) => {
    leadFormContainer.expect.element('@moveDateLabel').to.be.visible.after(0)
    leadFormContainer.expect.element('@moveDateLabel').text.to.equal('Move-in Date')
    leadFormContainer.expect.element('@moveDateInput').to.be.visible.after(0)
    leadFormContainer.expect.element('@moveDateInput').attribute('value').to.equal(moveDate)
  },
  ...
  verifyMoveReasonSelection: (leadFormContainer, moveReasonSelection) => {
    if (moveReasonSelection) {
      const unselectedMoveReasons = ['Financial', 'Relocation', 'FamilyEvent', 'JobChange', 'Other']

      if (unselectedMoveReasons.indexOf(moveReasonSelection) !== -1) {
        unselectedMoveReasons.splice(unselectedMoveReasons.indexOf(moveReasonSelection), 1)
      }
      unselectedMoveReasons.forEach(option => {
        leadFormContainer.expect.element(`@baths${option}Option`).to.not.have.attribute('selected')
      })
      if (moveReasonSelection !== '') {
        leadFormContainer.expect.element(`@baths${moveReasonSelection}Option`)
          .to.have.attribute('selected')
      }
    }
  },
  verifyMoveReasonSection: (leadForm, moveReasonSelection) => {
    const leadFormContainer = leadForm.section.leadFormContainer
    leadFormContainer.expect.element('@moveReasonLabel').to.be.visible.after(0)
    leadFormContainer.expect.element('@moveReasonLabel').text.to.equal('Reason for Moving')
    leadForm.verifyBathsSelection(leadFormContainer, moveReasonSelection)
  },
  ...
  validateLeadForm: (browser, args) => {
    ...
    browser.element('css selector', moveReasonLabelSelector, result => {
      if (result.status !== -1) {
        leadForm.verifyMoveReasonSection(leadForm, args.baths)
      }
    })
  ...
  enterMoveDate: (browser, moveDate) => {
    const leadForm = browser.page.large.emailLeadForm()
    const leadFormContainer = leadForm.section.leadFormContainer

    if (moveDate) {
      browser.element('css selector', moveDateInputSelector, result => {
        if (result.status !== -1) {
  ...
  fillOutLeadForm: (browser, args) => {
    ...
    if (args.moveDate) {
      leadForm.enterMoveDate(browser, args.moveDate)
    }
  },
  ...
  sections: {
    leadFormContainer: {
      ...
      elements: {
        ...
        closeModal: '[data-tag_section=lead_submission_form] [data-test-id=closeModal] use',
