...
module.exports = {
  ...
  'Switch to Map tab': browser => {
    browser.page.small.propertyCard().click('@mapButtonImage')
      .waitForElementVisible('@staticMapImage')
  },

  'Validate Map tab': browser => {
    browser.assert.urlEquals(`${browser.launchUrl}${basePdpPath}#map`)
    ...
    pdpCard.validateMap(browser)
  },

  'Save on Map Tab': browser => {
    const pdpCard = browser.page.small.propertyCard()
    pdpCard.savePropAndValidate(pdpCard)
  },

  'Unsave on Map Tab': browser => {
    const pdpCard = browser.page.small.propertyCard()
    pdpCard.unSavePropAndValidate(pdpCard)
  },

  'Open Share Map': browser => {
  ...
  'Validate Share Map': browser => {
    browser.page.small.propertyCard().validateShareOpen(browser, shareData, 'map')
  },

  'Close Share Map': browser => {
    browser.page.small.propertyCard().section.shareProperty
      .click('@closeButton').waitForElementNotPresent('@shareFacebook')
  },

  'Switch back to to Photos tab': browser => {
  ...
  'Validate Photo tab again': browser => {
