const listSearchPath = '/yakutat-ak/houses/'
const afterMapListSearchPath = '/apartments/Alaska/Yakutat/'
...
module.exports = {
  ...
  'Set laundry Filter': browser => {
    browser.page.small.filtersOverlay().section.laundrySelect
      .getLocationInView('@laundryOptionTwo').click('@laundryOptionTwo')
  },

  'Set AC Filter': browser => {
    browser.page.small.filtersOverlay()
      .getLocationInView('@ACOption').click('@ACOption')
  },
  ...
  'Validate Filtered SRP': browser => {
    ...
    filteredSRP.verifyAllListingsMatchName(browser, 'Three')
  },
  ...
  'Verify List Page has retained expected refinments': browser => {
    const expectedURL = [
      `${browser.launchUrl}`,
      `${afterMapListSearchPath}`,
    ...
    const filteredSRP = browser.page.small.listSearch()
    browser.assert.urlEquals(expectedURL)
    filteredSRP.verifyAllListingsWithinPriceRange(browser, 400, 1500)
    filteredSRP.verifyAllListingsMatchName(browser, 'Three')
  },
  ...
  'Search in Another City': browser => {
    const searchBar = browser.page.small.header().section.searchBar
    ...
    searchBar.waitForElementVisible('@firstSearchOption')
      .click('@firstSearchOption').waitForURLEquals(`${browser.launchUrl}${losAngelesSearchPath}`)
  },
